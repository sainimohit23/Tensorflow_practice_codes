...
def initialize_parameters():
    ...
    W1 = tf.get_variable('W1', shape=[1, 1, 1, 8], initializer=tf.contrib.layers.xavier_initializer())
    W2 = tf.get_variable('W2', shape=[3, 3, 8, 16], initializer= tf.contrib.layers.xavier_initializer())
    W3 = tf.get_variable('W3', shape=[1, 1, 16, 32], initializer= tf.contrib.layers.xavier_initializer())
    ...
    b3 = tf.get_variable('b3', shape=[32], initializer= tf.zeros_initializer())
...
def forward_propagation(X, parameters):
    ...
    Z5 = tf.contrib.layers.fully_connected(Z4, 20)
